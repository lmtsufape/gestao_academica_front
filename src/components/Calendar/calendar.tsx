...
const Calendar: React.FC<CalendarProps> = ({
  ...
  const handleAgendarSlot = async (horario: string) => {
    const result = await Swal.fire({
      title: "Escolha a modalidade",
      text: "Como será o atendimento?",
      icon: "question",
      iconColor: "#972E3F",
      showCancelButton: true,
      confirmButtonText: "Presencial",
      confirmButtonColor: "#972E3F",
      cancelButtonText: "Remoto",
    ...
    if (result.isConfirmed) {
      // Presencial
      onAgendar(selectedDayString, horario, "PRESENCIAL");
    } else if (result.dismiss === Swal.DismissReason.cancel) {
      // Remoto
      onAgendar(selectedDayString, horario, "REMOTO");
    } else {
  ...
  const handleEditarSlot = (horario: string) => {
    router.push(
      `/prae/cronogramas/editar/${selectedDayString}?horario=${horario}`,
    );
  };
    ...
    <main className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
        {daysInMonth.map((day, index) => {
          ...
          if (cronogramaDay) {
            vagasDisponiveis = cronogramaDay.slots.filter(
              (s) => !s.userScheduled,
            ).length;
          }
          ...
          return (
            ...
            >
              ...
              {cronogramaDay && cronogramaDay.slots.length > 0 ? (
                (() => {
                  const vagasDisponiveis = cronogramaDay.slots.filter(
                    (s) => !s.userScheduled,
                  ).length;
                  const vagasAgendadas = cronogramaDay.slots.filter(
                    (s) => s.userScheduled,
                  ).length;
      ...
      {userRole === "tecnico" && (
        <div className="mt-6 flex flex-wrap justify-end gap-2">
          ...
          <button
            onClick={() =>
              router.push(
                "/prae/agendamentos/calendario/agendamentos-por-aluno",
              )
            }
            className="px-4 py-2 bg-primary-500 text-white rounded-md"
          >
          ...
          <button
            onClick={() =>
              router.push("/prae/agendamentos/calendario/todos-cancelamentos")
            }
            className="px-4 py-2 bg-primary-500 text-white rounded-md"
          >
            Cancelamentos
      ...
      {userRole === "aluno" && (
        <div className="mt-6 flex justify-end gap-x-4">
          <button
            onClick={() =>
              router.push("/prae/agendamentos/calendario/meus-agendamentos")
            }
          ...
          <button
            onClick={() =>
              router.push("/prae/agendamentos/calendario/meus-cancelamentos")
            }
            className="px-4 py-2 bg-extra-150 text-white rounded-md hover:bg-extra-50 "
          >
            Meus Cancelamentos
      ...
      >
        <h2 className="text-xl font-bold mb-4">
          Agendamentos para {selectedDayString}
        </h2>
        <p className="text-md text-gray-700 mb-4">
          Tipo de atendimento:{" "}
          <span className="font-semibold">{selectedTipoAtendimento}</span>
        </p>
        <p className="text-md text-gray-700 mb-4">
          Modalidade:{" "}
          <span className="font-semibold">Escolhida no agendamento</span>
        </p>
            ...
            return (
              ...
              >
                <span
                  className={
                    "font-semibold " +
                    (isSlotPast ? "text-gray-300 line-through" : "")
                  }
                >
                  {slot.horario}
                </span>
        ...
        {userRole === "tecnico" && <div className="mt-4"></div>}
        <button
          onClick={closeModal}
