...
const cadastro = () => {
  ...
  const [cronogramasExistentes, setCronogramasExistentes] = useState<
    CronogramaExistente[]
  >([]);
  const [cronogramaOriginal, setCronogramaOriginal] =
    useState<CronogramaData | null>(null);
  ...
  const fetchCronogramasExistentes = async () => {
    try {
      ...
      const cronogramasFormatados: CronogramaExistente[] = cronogramas.map(
        (cronograma: any) => {
          ...
          return {
            ...
            data: dataISO,
            tipoAtendimentoId:
              cronograma.tipoAtendimento?.id || cronograma.tipoAtendimentoId,
          };
        },
      );
  ...
  const verificarCronogramaExistente = (
    data: string,
    tipoAtendimentoId: number,
  ): boolean => {
    ...
    return cronogramasExistentes.some((cronograma) => {
      ...
      if (isEditMode && cronogramaOriginal) {
        const pertenceAoOriginal =
          cronogramaOriginal.datas.includes(data) &&
  ...
  const verificarConflitos = (): string[] => {
    ...
    dadosPreenchidos.datas.forEach((data) => {
      if (
        verificarCronogramaExistente(data, dadosPreenchidos.tipoAtendimentoId)
      ) {
  ...
  const salvarRegistro = async () => {
    ...
    if (conflitos.length > 0) {
      toast.error(
        `Já existem cronogramas para as seguintes datas: ${conflitos.join(", ")}`,
      );
      return;
    }
    ...
    try {
      if (isEditMode) {
        ...
        const novasDatas = dadosPreenchidos.datas.filter(
          (data) => !datasOriginais.includes(data),
        );

        const datasRemovidas = datasOriginais.filter(
          (data) => !dadosPreenchidos.datas.includes(data),
        );

        
        for (const data of datasRemovidas) {
          const cronogramaParaRemover = cronogramasExistentes.find(
            (c) =>
              c.data === data &&
  ...
  const editarRegistro = async (item: any) => {
    try {
      ...
      const cronogramasDoMesmoTipo = cronogramasExistentes.filter(
        (c) => c.tipoAtendimentoId === dados.tipoAtendimento?.id,
      );
      ...
      const dadosCronograma = {
        ...
        datas: dataFormatada
          ? [
              dataFormatada,
              ...datasDoCronograma.filter((d) => d !== dataFormatada),
            ]
          : datasDoCronograma,
      };
  ...
  useEffect(() => {
    if (
      dadosPreenchidos.tipoAtendimentoId &&
            ...
            <div className="mt-4 p-3 bg-gray-50 rounded">
              ...
              <div className="flex flex-wrap gap-2">
                {dadosPreenchidos.datas.map((date, index) => {
                  const isDataOriginal =
                    cronogramaOriginal?.datas.includes(date);
                  const existeConflito = verificarCronogramaExistente(
                    date,
                    dadosPreenchidos.tipoAtendimentoId,
                  );
